...
<div class="container py-3">
    <div class="row">
        ...
        <div class="col-lg-12 category-content">
            ...
            <?php if (!empty($_SESSION['cart'])): ?> <!-- Выводится содержимое корзины -->
                ...
                <form class="row g-3" method="post" action="cart/checkout">
                    ...
                    <?php if (!isset($_SESSION['user'])): ?> <!-- Если пользователь не зарегистрирован, мы ему покажем форму регистрации -->
                        <div class="col-md-6 offset-md-3">
                            <div class="form-floating mb-3">
                                <input type="email" name="email" class="form-control" id="email" placeholder="name@example.com" value="<?= get_field_value('email') ?>"> <!-- Если будут ошибки, они будут показаны с помощью функции get_field_value -->
                    ...
                    <div class="col-md-6 offset-md-3"> <!-- Поле примечания к заказу (будет показываться в не зависимости от статуса авторизации пользователя) -->
                        <div class="form-floating mb-3">
                    ...
                    <div class="col-md-6 offset-md-3"> <!-- Кнопка отправки формы -->
                        <button type="submit" class="btn btn-danger"><?php __('cart_view_order_btn'); ?></button>
                    </div>
                </form>

                <?php
                if (isset($_SESSION['form_data'])) { // Если пользователь передал данные из формы в сессию по ключу 'form_data'
                    unset($_SESSION['form_data']); // Удаляем эти данные из сессии по ключу 'form_data'
                }
                ?>

            <?php else: ?>
                <h4 class="text-start"><?php __('tpl_cart_empty'); ?></h4> <!-- Если корзина пуста, пользователь очистил корзину или попал на эту страницу случайно - выводим сообщение 'Корзина пуста' -->
            <?php endif; ?>

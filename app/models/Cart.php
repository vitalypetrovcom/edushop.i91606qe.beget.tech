...
class Cart extends AppModel { // Модель для работы с корзиной товаров
    ...
    public static function translate_cart ($lang) { // Метод для автоматического изменения названия товаров, при переключении языков пользователем (с RU на EN и обратно) в корзине в модальном окне. Принимает в качестве переменной язык "$lang", на который нужно перевести корзину
        if (empty($_SESSION['cart'])) { // Проверка, если у нас пусто в "$_SESSION['cart']" (корзина пуста), тогда выходим из данной функции (ничего делать не надо)
            return;
        }
        $ids = implode (',', array_keys ($_SESSION['cart'])); // Иначе, мы должны получить номера $id товаров в корзине используя функцию "implode" (соберет все значения в строку по разделителю ',' с ключами массива array_keys ($_SESSION['cart'])).
        $products = R::getAll ("SELECT p.id, pd.title FROM product p JOIN product_description pd on p.id = pd.product_id WHERE p.id IN ($ids) AND pd.language_id = ?", [$lang['id']]);  // Получим продукты в переменную $products используя функцию RedBeanPHP getAll. В $products должен быть массив с данными (p.id и pd.title)
        foreach ($products as $product) { // Проходим в цикле по массиву $products и заменяем (перезаписываем) значение 'title' в массиве $_SESSION['cart'][$product['id']]['title'] на значение из массива $product['title']

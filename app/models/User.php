...
class User extends AppModel { // Модель (класс) для работы с данными пользователей из БД
    ...
    public array $rules = [ // Здесь мы укажем массив с правилами
        ...
        'optional' => ['email', 'password'], // Данная опция относится к настройкам валидатора valitron. Если для формы указанные поля ('email', 'password') не будут заполнены значениями пользователем, тогда они не будут участвовать в процессе валидации (правила 'required' и 'email' к ним применяться не будут)
    ];

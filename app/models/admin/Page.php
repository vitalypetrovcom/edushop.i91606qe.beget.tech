...
class Page extends AppModel { // Модель (класс) для работы со страницами сайта в админ-панели
    ...
    public function get_page ($id): array { // Модель для получения данных страницы из БД

        // Нам нужно получить ассоциативный массив
        return R::getAssoc ("SELECT pd.language_id, pd.*, p.* FROM page_description pd JOIN page p on p.id = pd.page_id WHERE pd.page_id = ?", [$id]); // В качестве ключа ассоциативного массива используем pd.language_id и выбираем все данные из таблиц page_description и page БД

    }

    public function update_page ($id): bool { // Метод для сохранения страницы после редактирования данных из админ-панели в БД. Принимает на вход $id страницы, которой нужно обновить после редактирования

        R::begin (); // Используем транзакцию RedBeanPHP используя try-catch
        try {

            // Выгружаем данные в таблицу page
            $page = R::load ('page', $id); // Получаем объект страницы по $id из БД
            if (!$page) { // Если $id страницы не верный или мы по другим причинам не получили данные о странице
                return false;
            }
            ...
            foreach ($_POST['page_description'] as $lang_id => $item) { // После этой операции, мы можем пройтись по массиву page_description, чтобы получить нужные данные
                R::exec ("UPDATE page_description SET title = ?, content = ?, keywords = ?, description = ? WHERE page_id = ? AND language_id = ?", [
                    ...
                    $id,
                    $lang_id,
                ]); // Выполняем запрос на обновление данных страницы в таблице page_description БД
            }

...
class CategoryController extends AppController { // Контроллер (класс) для обработки страниц категорий товаров
    ...
    public function viewAction () { // Метод для получения данных категории и данные товаров, относящихся к данной категории
        ...
        $page = abs(get ('page')) ?: 1; // Получаем номер страницы используя функцию get по ключу 'page'. Используем оператор abs, чтобы пользователь через адресную строку не смог передать отрицательное число. Если функция get вернет false, тогда мы запишем в переменную 'page' значение 1. Вернем номер страницы (если он корректный) ИЛИ вернем 1.

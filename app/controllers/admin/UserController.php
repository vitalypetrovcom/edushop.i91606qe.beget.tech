...
class UserController extends AppController { // Контроллер для работы с пользовательской части админки
    ...
    public function addAction () { // Метод для добавления нового пользователя в админ-панели

        // Если данные к нам будут приходить POST
        if (!empty($_POST)) { // Если у нас массив POST не пустой
            $this->model->load (); // Загружаем данные в атрибуты модели
            if (!$this->model->validate ($this->model->attributes) || !$this->model->checkUnique ('Этот Email уже занят!')) { // Если валидация атрибутов не была пройдена ИЛИ если пользователь не прошел проверку в модели checkUnique ('') (был введен некорректный Email, который уже есть в БД у другого пользователя) - выдаем ошибку 'Этот Email уже занят!'
                ...
                $_SESSION['form_data'] = $_POST; // Запишем в сессию данные заполненной формы из массива POST
            } else { // Если все проверки прошли успешно
                $this->model->attributes['password'] = password_hash ($this->model->attributes['password'], PASSWORD_DEFAULT); // Мы хешируем пароль стандартным типом PASSWORD_DEFAULT
                if ($this->model->save ('user')) { // Сохраняем пользователя. У нас только одна таблица, поэтому используем метод save в таблицу 'user' БД
                    $_SESSION['success'] = 'Пользователь добавлен!'; // Выдаем сообщение об успехе
                } else {
                    $_SESSION['errors'] = 'Ошибка при добавлении пользователя!'; // Выдаем сообщение об ошибке
                }
        ...
        // Если данные к нам будут приходить GET
        $title = 'Новый пользователь'; // Объявляем переменную $title
        $this->setMeta ("Админка :: {$title}"); // Передаем название title в представление

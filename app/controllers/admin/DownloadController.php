...
class DownloadController extends AppController { // Контроллер (класс) для работы с загружаемыми файлами для прикрепления их к цифровым товарам с админ-панели
    ...
    public function deleteAction () { // Метод для удаления файлов цифровых товаров

        $id = get ('id'); // Получаем id файла (берем его из массива GET)
        if (R::count ('order_download', 'download_id = ?', [$id])) { // Если товар с таким id был заказан пользователем (мы получили хотя бы одну запись), тогда такой файл мы удалять не будем давать возможность (пользователь его уже приобрел и нельзя удалять приобретенный файл)
            $_SESSION['errors'] = 'Невозможно удалить - данный файл уже был куплен!'; // В массив $_SESSION['errors'] запишем сообщение об ошибке
            redirect (); // Делаем редирект на ту же страницу
        }
        if (R::count ('product_download', 'download_id = ?', [$id])) { // Если товар с таким id был заказан пользователем (мы получили хотя бы одну запись), тогда такой файл мы удалять не будем давать возможность (данный файл прикреплен к товару)
            $_SESSION['errors'] = 'Невозможно удалить - данный файл прикреплен к товару!'; // В массив $_SESSION['errors'] запишем сообщение об ошибке
            redirect (); // Делаем редирект на ту же страницу
        }

        if ($this->model->download_delete ($id)) { // Вызываем метод модели для удаления файла по его $id
            $_SESSION['success'] = 'Файл успешно удален!'; // В массив $_SESSION['success'] запишем сообщение об успехе
        } else {
            $_SESSION['errors'] = 'Ошибка удаления файла!'; // В массив $_SESSION['errors'] запишем сообщение об ошибке
        }
        redirect (); // Вида нам здесь не нужно. Делаем редирект на ту же страницу
    }

...
class Registry { // Класс Реестр - необходим для сбора(записи) всех данных от запросов пользователя в контейнер
    ...
    public function setProperty ($name, $value) { // Для реализации шаблона "Реестр", метод "setProperty" будет записывать в контейнер какие-либо данные, а метод "getProperty" будет получать данные из контейнера, если они есть

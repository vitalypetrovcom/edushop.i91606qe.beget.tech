...
function h ($str) { // Функция для обработки данных, которые могут прийти динамически: там где не нужно выполнение html кода, данные от пользователя или неизвестного источника (могут содержать вредоносный код - XSS уязвимость) - их обрабатывают с помощью htmlspecialchars или htmlentities
    ...
    return htmlspecialchars ($str, ENT_QUOTES); // До версии PHP 8.1.0 - По умолчанию, значение флага установлено в ENT_COMPAT (Преобразует двойные кавычки, одинарные кавычки не изменяются). Если нам нужно иметь возможность преобразовывать и двойные и одинарные кавычки, устанавливаем для флага значение ENT_QUOTES (Преобразует как двойные, так и одинарные кавычки).
    // Начиная с версии PHP 8.1.0 - Значение по умолчанию параметра flags изменено с ENT_COMPAT на ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401.
